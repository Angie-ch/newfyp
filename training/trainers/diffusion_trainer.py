...
Trains the physics-informed multi-task diffusion model
"""
...
class DiffusionTrainer:
    """
    Trainer for physics-informed multi-task diffusion model
    """
    ...
    def __init__(
        ...
        # Logging
        self.log_dir = Path(config.get('log_dir', 'logs/diffusion'))
    ...
    def train_epoch(self, epoch: int) -> dict:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with torch.no_grad():
                B_t, T_in, C, H, W = past_frames.shape
                past_flat = past_frames.reshape(B_t * T_in, C, H, W)
                past_latents = self.autoencoder.encode(past_flat)
                past_latents = past_latents.reshape(B_t, T_in, -1, H // 8, W // 8)
                
                T_out = future_frames.shape[1]
                future_flat = future_frames.reshape(B_t * T_out, C, H, W)
                future_latents = self.autoencoder.encode(future_flat)
                future_latents = future_latents.reshape(B_t, T_out, -1, H // 8, W // 8)
    ...
    def validate(self, epoch: int) -> dict:
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Validating"):
                ...
                # Forward pass (use EMA model)
                predictions = self.ema_model(noisy_latents, t, condition_dict, return_x0=True)
    ...
    def save_checkpoint(self, epoch: int, val_loss: float, is_best: bool = False):
        ...
        checkpoint = {
            ...
            'diffusion_state': {
                ...
                'alphas_bar': self.diffusion.alphas_bar
            },
            'val_loss': val_loss,
            'config': self.config
        }
